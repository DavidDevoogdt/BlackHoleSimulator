...
impl std::fmt::Display for XYZ {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f,"{}nm, [{:.04},{:.04},{:.04}]",self.lambda,self.X,self.Y,self.Z)
    }
}
...
impl CieLookup{
    ...
    pub fn wavelength_to_rgb(&self, lambda: f64 ) -> image::Rgb<u8> {
        ...
        return Self::xyz_to_rgb(  frac*xyz1.X + (1.0-frac)*xyz2.X , frac*xyz1.Y + (1.0-frac)*xyz2.Y, frac*xyz1.Z + (1.0-frac)*xyz2.Z);
    }
