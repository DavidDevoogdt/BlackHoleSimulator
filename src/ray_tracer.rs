...
impl<'a, T: curved_space::Metric<'a>> RayTracer<'a, T> {
    pub fn run_simulation(&mut self, max_tolerance: f64) {
        self.photons
            .par_iter_mut()
            .progress_count((self.camera.x_res * self.camera.y_res) as u64)
            .for_each(|photon| {
                ...
                'outer: while photon.steps_left > 0 {
                    ...
                    if new_err < max_tolerance {
                        ...
                        let [coord, mom] = photon
                            .dynamic
                            .calculate_coordinates_and_contravariant_momenta();
                        ...
                        if photon.save_path {
                            photon
                                .path
                                .push(photon.dynamic.calculate_cartesian_coordinates_and_momenta());
                        }
                    } else {
                        photon.dynamic.restore_state();
                        photon
                            .dynamic
                            .set_d_lambda(Some(0.5 * photon.dynamic.get_d_lambda()));
                    }
                }
            });
    }
    ...
    pub fn plot_paths(&self, mode: &str) {
        if self.save_path {
            for (i, photon) in self.photons.iter().enumerate() {
                ...
                let _ =
                    python_interface::save_to_csv(&photon.path, format!("files/photon{}.csv", i));
            }
    ...
    pub fn generate_image(&self, name: &str) {
        let mut img: RgbImage =
            ImageBuffer::new(self.camera.x_res as u32, self.camera.y_res as u32);
...
pub fn new<'a, T: curved_space::Metric<'a>>(
    camera: Camera,
    objects: &'a Vec<Box<dyn CollsionObject>>,
    metric: &'a T,
    max_steps: i32,
    save_path: bool,
) -> RayTracer<'a, T> {
    let r =
        (camera.direction[0].powi(2) + camera.direction[1].powi(2) + camera.direction[2].powi(2))
            .sqrt();
    ...
    let theta_x =
        (alpha_x * camera.rotation_angle.cos() - beta_x * camera.rotation_angle.sin()) * (dheigth);
    let theta_y =
        (alpha_y * camera.rotation_angle.cos() - beta_y * camera.rotation_angle.sin()) * (dheigth);
    let theta_z = (alpha_z * camera.rotation_angle.cos()) * (dheigth);

    let phi_x =
        (alpha_x * (camera.rotation_angle.sin()) + beta_x * camera.rotation_angle.cos()) * (dwidth);
    let phi_y =
        (alpha_y * (camera.rotation_angle.sin()) + beta_y * camera.rotation_angle.cos()) * (dwidth);
    ...
    for x in 0..camera.x_res {
        for y in 0..camera.y_res {
            ...
            let obj = metric.spawn_space_object_from_cartesian(
                pos,
                [1.0, px / norm, py / norm, pz / norm],
                0.0,
            );
...
//This creates the initial photons for the chosen metric and associated space object
pub fn new_parallel<'a, T: curved_space::Metric<'a>>(
    metric: &'a T,
    objects: &'a Vec<Box<dyn CollsionObject>>,
    direction: [f64; 3],
    center: [f64; 3],
        ...
        if dtheta1 * dtheta2 < 0.0 {
            if (dtheta1 - dtheta2).abs() <= 0.5 {
                ...
                if r >= self.radius1 && r <= self.radius2 {
                    ...
                    let rfact = ((self.divisions_radial as f64) * (r - self.radius1)
                        / (self.radius2 - self.radius1))
                        .round() as i32;
                    let p = ((self.divisions_angular as f64)
    ...
    fn detect_collision(
        ...
        if r >= self.radius1 && r <= self.radius2 {
            ...
            if (std::f64::consts::PI / 2.0 - theta1) * (std::f64::consts::PI / 2.0 - theta2) < 0.0 {
                ...
                let p =
                    (self.divisions_radial as f64 * phi / (std::f64::consts::PI).round()) as i64;
...
impl CollsionObject for SkyboxKerr {
    fn detect_collision(
        ...
        if r > self.radius {
            ...
            let [_, cart_mom] = curved_space::kerr_to_cart(
                &[new[1], new[2], new[3]],
            ...
            let xcoor = (((self.x_res as f64)
                * ((phi + self.phi_offset + std::f64::consts::PI) / (2.0 * std::f64::consts::PI)))
                .round() as i32)
                .rem_euclid(self.x_res);
            let ycoor = (((self.y_res as f64) * ((theta) / (std::f64::consts::PI))).round() as i32)
                .rem_euclid(self.y_res);
...
impl CollsionObject for Skybox {
    fn detect_collision(
        ...
        if r > self.radius {
            let [_, cart_mom] = curved_space::spher_to_cart(
                &[new[1], new[2], new[3]],
                &[momentum[1], momentum[2], momentum[3]],
            );
            ...
            let xcoor = (((self.x_res as f64)
                * ((phi + self.phi_offset) / (2.0 * std::f64::consts::PI)))
                as i32)
                .rem_euclid(self.x_res);
