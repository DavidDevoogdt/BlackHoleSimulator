...
pub fn launch_parallel_photons_kerr() {
    ...
    let col_objects: Vec<Box<dyn ray_tracer::CollsionObject>> =
        vec![Box::new(black_sphere), Box::new(skybox)];
...
pub fn ray_trace_schwarzshild() {
    ...
    let mut ray_tracer = ray_tracer::new(camera, &col_objects, &metric, steps, false);
...
pub fn ray_trace_minkowski() {
    ...
    let metric = curved_space::new_minkowski_metric(0.01);
...
pub fn photon_orbit() {
    ...
    let mut ray_tracer = ray_tracer::new(camera, &col_objects, &metric, 20000, true);
...
pub fn test_coordinate_system_2() {
    ...
    let [coor, mom] = [[0.0, -1.3, 2.5, 0.4], [1.0, 0.0, 1.0, 0.0]];
    ...
    println!(
        "[{:.4},{:.4},{:.4},{:.4}]->[{:.4},{:.4},{:.4},{:.4}]",
        coor[0], coor[1], coor[2], coor[3], coor2[0], coor2[1], coor2[2], coor2[3]
    );
    println!(
        "[{:.4},{:.4},{:.4},{:.4}]->[{:.4},{:.4},{:.4},{:.4}]",
        mom[0], mom[1], mom[2], mom[3], mom2[0], mom2[1], mom2[2], mom2[3]
    );
}

pub fn test_coordinate_system_3() {
    let [coor, mom] = [[-1.3, 2.31, 0.4], [1.0, 3.0, -0.68]];
    ...
    println!(
        "[{:.4},{:.4},{:.4}]->[{:.4},{:.4},{:.4}]",
        coor[0], coor[1], coor[2], coor3[0], coor3[1], coor3[2]
    );
    println!(
        "[{:.4},{:.4},{:.4}]->[{:.4},{:.4},{:.4}]",
        mom[0], mom[1], mom[2], mom3[0], mom3[1], mom3[2]
    );
}
...
pub fn test_coordinate_system() {
    ...
    let [coor, mom] = [[1.0, -1.28, 2.85, 0.4], [1.0, 1.0, 3.1, -0.68]];
    ...
    println!(
        "[{:.4},{:.4},{:.4}{:.4}]->[{:.4},{:.4},{:.4},{:.4}]",
        mom[0], mom[1], mom[2], mom[3], contra_mom[0], contra_mom[1], contra_mom[2], contra_mom[3]
    );
}
